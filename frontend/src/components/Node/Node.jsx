...
const getGlobalReferences = (nodes) => {
  ...
  refNodes.forEach((pn) => {
    ...
    const preview = pn.data.previewUrl || pn.data.resultUrl || pn.data.generatedImageUrl || '';
    refs.push({
      name: refName,
      nodeId: pn.id,
      label: label,
      preview: preview,
      filename: pn.data.filename || `${pn.type.replace('meta_', '')}_output.jpg`,
...
const Node = ({ node, transform }) => {
  ...
  useLayoutEffect(() => {
    ...
    const observer = new ResizeObserver((entries) => {
      for (const entry of entries) {
        ...
        // Thực hiện cộng thêm padding/border để lấy chính xác kích thước thật (border-box)
  ...
  const getIcon = () => {
    switch (node.type) {
      ...
      case 'file_input': return <Zap size={18} color="#ec4899" />;
      ...
      <>
        ...
        {resultUrl && (
            <div className="node-custom-ui result-view" onMouseDown={e => e.stopPropagation()}>
                ...
                ) : ((resultUrl.includes('.mp4') || node.type === 'meta_video_gen' || node.type === 'meta_video' || node.type === 'merge_videos') && resultUrl.startsWith('http')) ? (
                    <video 
                        src={resultUrl} 
